...
include 'db_connect.php';
	...
	if($qry->num_rows > 0){
		...
		$employee = ucwords($emp['firstname'].' '.$emp['middlename'].' '.$emp['lastname']);
		if($type == 1){
			$log = ' time in the morning';
		}elseif($type == 2){
			$log = ' time out the morning';
		}elseif($type == 3){
			$log = ' time in the afternoon';
		}elseif($type == 4){
			$log = ' time out the afternoon';
		}
		if($save_log){
				$data['status'] = 1;
